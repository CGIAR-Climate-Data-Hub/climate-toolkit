...
async def download_data(request: DataFetchRequest, settings=Depends(get_settings)):
    """Fetch data and return it as a downloadable file.

    - Single source/model -> one CSV (or JSON) attachment.
    - Multiple NEX-GDDP models ('models' list or ['all']) -> a ZIP of
      per-model files (<source>_<model>.csv), mirroring the CLI's
      one-file-per-model behaviour.
    """
    try:
        source_enum = getattr(ClimateDataset, request.source, None)
        if not source_enum:
            raise HTTPException(
                status_code=400,
                detail=f"Unknown source: {request.source}. Available: {[s.key for s in AVAILABLE_SOURCES]}",
            )
        ...
        date_from = datetime.strptime(request.date_from, "%Y-%m-%d").date()
        date_to = datetime.strptime(request.date_to, "%Y-%m-%d").date()
        fmt = request.format if request.format in ("csv", "json") else "csv"
        ...
        def _serialize(df):
            if fmt == "json":
                return df.to_json(orient="records", date_format="iso", indent=2), "json"
        ...
        if request.models:
            ...
            with zipfile.ZipFile(buf, "w", zipfile.ZIP_DEFLATED) as zf:
                for m in model_list:
                    content, ext = _serialize(_frame(m))
                    zf.writestr(f"{request.source}_{m}.{ext}", content)
            buf.seek(0)
            fname = f"{request.source}_{request.date_from}_{request.date_to}_models.zip"
        ...
        # Single file.
        content, ext = _serialize(_frame(request.model))
        media = "application/json" if ext == "json" else "text/csv"
        fname = f"{request.source}_{request.date_from}_{request.date_to}.{ext}"
        return Response(
