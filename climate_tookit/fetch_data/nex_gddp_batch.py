...
def _safe_site_fragment(name: str, max_length: int = 24) -> str:
...
def load_sites(
    ...
    for site in sites or []:
        resolved.append(_normalize_site(site))
    ...
    for site in resolved:
        key = (
            site.name,
            _normalize_cache_coord(site.lat),
            _normalize_cache_coord(site.lon),
        )
        if key not in seen:
            seen.add(key)
            deduped.append(site)
...
def initialize_earth_engine(
    ...
    resolved_project_id = _infer_ee_project_id(project_id)
...
def _site_batch_digest(site_batch: list[Site]) -> str:
    payload = "|".join(
        f"{site.name}:{_normalize_cache_coord(site.lat):.{CACHE_COORD_DECIMALS}f}:"
        f"{_normalize_cache_coord(site.lon):.{CACHE_COORD_DECIMALS}f}"
        for site in site_batch
    )
...
def cache_paths_for_batch(
    *,
    ...
    base = Path(cache_dir) / CACHE_SCHEMA_VERSION / scenario / model
    if len(site_batch) == 1:
        site_fragment = _safe_site_fragment(site_batch[0].name)
    else:
        site_fragment = (
            f"{_safe_site_fragment(site_batch[0].name)}_to_"
            f"{_safe_site_fragment(site_batch[-1].name)}"
        )
    filename = (
