...
| `auto` | pass | now follows `era_5 -> agera_5 -> chirps_v2+chirts` fallback, aligned with `seasons.py` intent | `analysis/stress_stats_auto.json` |
| `chirps_v2` | pass with fallback | precip-only source; module injects `tmax=25`, `tmin=15` by design | `analysis/stress_stats_chirps.json` |
...
| `chirps_v2+chirts` | clean reject | valid paired-source concept, but this 2018-2019 run is unavailable because CHIRTS daily coverage ends in 2016 | `analysis/stress_stats_chirps_chirts.json` |
...
1. `auto` was inconsistent with `seasons.py`.
   It used to route straight to `chirps_v2+chirts`, which breaks for post-2016 windows because CHIRTS daily ends in 2016.

2. `chirps_v2+chirts` failure in 2018-2019 is expected from dataset coverage, not random fetch instability.
...
4. `imerg`, `chirps_v2`, and `chirts` need explicit pairing logic if they should be handled cleanly here.
...
- `statistics.py`
  - added source compatibility guards for `terraclimate`, `imerg`, `chirts`, and post-2016 `chirps_v2+chirts`
  - changed `auto` to try `era_5`, then `agera_5`, then `chirps_v2+chirts`
