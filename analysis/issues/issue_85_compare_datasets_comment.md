...
`climate_toolkit/compare_datasets/compare_datasets.py` currently says its outputs are `"not synthetic mock data"`, but when `nex_gddp` is included it still routes through the active placeholder `nex_gddp` backend via `preprocess_data(...)`.
