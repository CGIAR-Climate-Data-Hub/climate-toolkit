...
from climate_toolkit.fetch_data.runtime_notes import build_historical_cache_note
from climate_toolkit.fetch_data.source_data.sources.utils.models import (
    ClimateVariable,
    normalize_climate_dataset_name,
)
from climate_toolkit.crop_calendar.ggcmi import (
...
from climate_toolkit.weather_station.overrides import apply_custom_station_overrides
...
def detect_onset_cessation(df, verbose=True):
...
def reassign_spillover_seasons(results_dict, lat=0, start_year=None,
                                end_year=None, hemisphere="NH", verbose=True):
    if 8 <= lat <= 20:
...
def remove_duplicate_seasons(refined_results, verbose=True):
    deduped = {}
...
def fetch_master_range_with_tail(
    lat,
    lon,
    start_year,
    end_year,
    ...
    tail_end_dt = (date(end_year, 12, 31) + pd.DateOffset(months=extra_months)).date()
    end_date = tail_end_dt.strftime("%Y-%m-%d")
    print(f"  Fetching master fallback window {start_date} to {end_date} ...")
    df = get_climate_data(
        lat,
        lon,
        start_date,
...
def analyze_years_fixed_on_prefetched_df(
    df: pd.DataFrame,
    ...
    for year in range(start_year, end_year + 1):
        print(f"\nFixed-season year {year} | using prefetched master window")
        ...
        for sd in fixed_seasons:
            ...
            if eto_seasons:
                print(
                    ...
                    f"ETO sub-seasons={len(eto_seasons)}"
                )
            else:
                print(
                    f"  Fixed window: {onset_date.strftime('%Y-%m-%d')} → "
                    f"{cessation_date.strftime('%Y-%m-%d')} | {length_days}d | "
                    f"ETO: {eto_detection_note or 'none detected'}"
                )
...
def analyze_years_auto_on_prefetched_df(
    df: pd.DataFrame,
    lat: float,
    start_year: int,
    end_year: int,
    ...
    for ref_year in range(start_year, end_year + 1):
        ...
        df_window = (
            df[(df['date'] >= win_start) & (df['date'] <= win_end)]
            .copy()
            .reset_index(drop=True)
        )
        try:
            ...
            seasons_dict[ref_year] = []
            annual_dict[ref_year] = {'error': str(e)}

    temp = reassign_spillover_seasons(
        seasons_dict,
        lat=lat,
...
def main() -> int:
    ...
    parser.add_argument('--crop', default=None,
                        help='Optional crop name for crop-calendar fallback, e.g. maize or rice')
    ...
    total_detected = 0
    if args.crop or args.calendar_source:
        try:
            if not (args.crop and args.calendar_source):
                raise ValueError("Provide both --crop and --calendar-source together.")
            requested_calendar_preset = resolve_calendar_preset(
                lat,
                lon,
                args.crop,
    ...
    if args.fixed_season:
        ...
        if requested_calendar_preset:
            master_df = fetch_master_range_with_tail(
                lat,
                lon,
                start_year=args.start_year,
                end_year=args.end_year,
            ...
            total_detected = sum(len(v or []) for v in seasons_dict.values())
            if total_detected == 0:
                print(
                    "Auto detection returned no usable seasons. "
    ...
    save_path = None
    if not args.no_save:
        mode     = "fixed" if (args.fixed_season or auto_fallback_applied) else args.source
...
# Automatic detection:
# python climate_toolkit/season_analysis/seasons.py --location="-1.286,36.817" --start-year 2018 --end-year 2020 --source era_5
# python climate_toolkit/season_analysis/seasons.py --location="-1.286,36.817" --start-year 2018 --end-year 2020 --source agera_5
# python climate_toolkit/season_analysis/seasons.py --location="-1.286,36.817" --start-year 2015 --end-year 2016 --source chirps_v2+chirts
# python climate_toolkit/season_analysis/seasons.py --location="-1.286,36.817" --start-year 2015 --end-year 2020 --source agera_5 --output-dir ./results

# Fixed season:
# python climate_toolkit/season_analysis/seasons.py --location="-1.286,36.817" --start-year 2015 --end-year 2022 --fixed-season "03-01:05-31" --source era_5

# Fixed two seasons:
# python climate_toolkit/season_analysis/seasons.py --location="-1.286,36.817" --start-year 2015 --end-year 2022 --fixed-season "03-01:05-31,10-01:12-15" --source agera_5

# Fixed year-crossing season:
# python climate_toolkit/season_analysis/seasons.py --location="-1.286,36.817" --start-year 2015 --end-year 2022 --fixed-season "11-01:02-28" --source chirps_v2+chirts

# Fixed season, auto source selection, no CSV:
# python climate_toolkit/season_analysis/seasons.py --location="-1.286,36.817" --start-year 2018 --end-year 2022 --fixed-season "04-15:07-10" --source auto --no-save
